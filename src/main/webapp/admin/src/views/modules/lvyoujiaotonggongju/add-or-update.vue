<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="工具名称" prop="gongjumingcheng">
          <el-input v-model="ruleForm.gongjumingcheng" 
              placeholder="工具名称" clearable  :readonly="ro.gongjumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="工具名称" prop="gongjumingcheng">
              <el-input v-model="ruleForm.gongjumingcheng" 
                placeholder="工具名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='gongjumingcheng'){
            this.ruleForm.gongjumingcheng = obj[o];
	    this.ro.gongjumingcheng = true;
            continue;
          }
          if(o=='leixing'){
    ...
    info(id) {
      this.$http({
        url: `lvyoujiaotonggongju/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lvyoujiaotonggongju/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lvyoujiaotonggongjuCrossAddOrUpdateFlag = false;
						 this.parent.search();
