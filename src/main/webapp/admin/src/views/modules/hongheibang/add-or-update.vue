    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="店铺位置" prop="dianpuweizhi">
          <el-input v-model="ruleForm.dianpuweizhi" 
              placeholder="店铺位置" clearable  :readonly="ro.dianpuweizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="店铺位置" prop="dianpuweizhi">
              <el-input v-model="ruleForm.dianpuweizhi" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="游客评语" prop="youkepingyu">
          <el-input v-model="ruleForm.youkepingyu" 
              placeholder="游客评语" clearable  :readonly="ro.youkepingyu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="游客评语" prop="youkepingyu">
              <el-input v-model="ruleForm.youkepingyu" 
                placeholder="游客评语" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="榜类型" prop="bangleixing">
          <el-select :disabled="ro.bangleixing" v-model="ruleForm.bangleixing" placeholder="请选择榜类型">
            <el-option
                v-for="(item,index) in bangleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="榜类型" prop="bangleixing">
	      <el-input v-model="ruleForm.bangleixing"
                placeholder="榜类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.dianpuweizhi = true;
          ...
          if(o=='youkepingyu'){
            this.ruleForm.youkepingyu = obj[o];
	    this.ro.youkepingyu = true;
            continue;
          }
          if(o=='pingfen'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='bangleixing'){
            this.ruleForm.bangleixing = obj[o];
	    this.ro.bangleixing = true;
            continue;
          }
          if(o=='fabushijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.bangleixingOptions = "红榜,黑榜".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `hongheibang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `hongheibang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.hongheibangCrossAddOrUpdateFlag = false;
						 this.parent.search();
